...
Communaute::Communaute():Case(){}

Communaute::Communaute(string nom, int numero):Case(nom, numero){}

void Communaute::arreterSur(Joueur* joueur, bool fast_game){
    cout << "Carte Communaute:" << endl;

    if (!fast_game)
        sleep_for(1000ms);
    srand((unsigned int)time(0));
    Carte carte = cartes.get_carte_communautte(rand()%17);
    ...
    if (carte.get_action_special() == 3){
        ...
        cout << "Vous devez payer: " << prix << " pour les travails dans vos maisons et hotels." << endl;
        joueur->debiter(-prix);
    }
